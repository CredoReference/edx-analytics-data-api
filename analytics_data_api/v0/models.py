...
class RosterEntry(DocType):
    ...
    def get_users_in_course(
        ...
        if segments and ignore_segments:
            raise ValueError('Can not combine `segments` and `ignore_segments` parameters.')

        order_by = order_by or 'username'
        ...
        # Sorting
        order_by_options = (
            'username', 'email', 'discussions_contributed', 'problem_attempts', 'problems_attempted', 'problems_completed', 'videos_viewed'
        )
